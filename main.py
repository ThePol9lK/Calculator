...
class Calculator(QMainWindow):
    def __init__(self):
        ...
        # math
        self.ui.btn_equa.clicked.connect(self.calculate)
    ...
    @staticmethod
    def remote_trailing_zeros(num: str) -> str:
        n = str(float(num))
        return n[:-2] if n[-2:] == '.0' else n

    def add_temp(self) -> None:
        btn = self.sender()
        entry = self.remote_trailing_zeros(self.ui.le_entry.text())
    ...
    def calculate(self) -> Optional[str]:
        ...
        if temp:
            try:
                result = self.remote_trailing_zeros(
                    str(operations[self.get_math_sign()](Decimal(self.get_temp_num()), Decimal(self.get_entry_num())))
                )

                self.ui.lbl_temp.setText(temp + self.remote_trailing_zeros(entry) + ' =')
                self.adjust_temp_font_size()
    ...
    def remove_error(self) -> None:
        if self.ui.le_entry.text() in (error_underfined, error_zero_div):
            self.ui.le_entry.setMaxLength(self.entry_max_len)
            self.ui.le_entry.setText('0')
            self.adjust_entry_font_size()
    ...
    def disable_buttons(self, disable: bool) -> None:
        self.ui.btn_equa.setDisabled(disable)
        self.ui.btn_plus.setDisabled(disable)
        self.ui.btn_min.setDisabled(disable)
    ...
    def change_buttons_color(self, css_color: str) -> None:
        self.ui.btn_equa.setStyleSheet(css_color)
        self.ui.btn_plus.setStyleSheet(css_color)
        self.ui.btn_min.setStyleSheet(css_color)
